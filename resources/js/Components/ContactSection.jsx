            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    ...
                    <motion.div 
                        ...
                        viewport={{ once: true }}
                    >
                        <h2 className="text-3xl md:text-5xl font-bold text-brand-text mb-6">{t.contact.title}<span className="text-gradient">{t.contact.title_gradient}</span></h2>
                        <p className="text-brand-text-muted mb-10 text-lg">{t.contact.desc}</p>
                        
                        <div className="space-y-6 mb-12">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full glass-panel flex items-center justify-center text-brand-green bg-white shadow-sm border border-gray-100">
                                    <MessageCircle size={20} />
                                </div>
                                <div>
                                    <p className="text-sm text-gray-500">WhatsApp</p>
                                    <a href={`https://wa.me/6285175020319?text=${encodeURIComponent(t.hero.wa_text)}`} target="_blank" rel="noreferrer" className="text-lg font-medium text-brand-text hover:text-brand-green transition-colors">+62 851-7502-0319</a>
                                </div>
                            </div>
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full glass-panel flex items-center justify-center text-brand-green bg-white shadow-sm border border-gray-100">
                                ...
                                <div>
                                    <p className="text-sm text-gray-500">{t.contact.location_title}</p>
                                    <p className="text-lg font-medium text-brand-text">{t.contact.location}</p>
                                </div>
                            </div>
                        </div>

                        <div className="flex gap-4">
                            <a href="https://github.com/yulianasetianingrum" target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full glass-panel flex items-center justify-center text-gray-600 hover:text-brand-green hover:bg-gray-50 transition-all border border-gray-200">
                                <GithubIcon />
                            </a>
                            <a href="https://www.linkedin.com/in/yuliana-setianingrum" target="_blank" rel="noreferrer" className="w-10 h-10 rounded-full glass-panel flex items-center justify-center text-gray-600 hover:text-[#0A66C2] hover:bg-gray-50 transition-all border border-gray-200">
                                <LinkedinIcon />
                            </a>
                        </div>
                    </motion.div>
